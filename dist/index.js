...
async function updateQuery(params) {
    try {
        ...
        if (updateData.name !== undefined)
            queryData.name = updateData.name;
        if (updateData.data_source_id !== undefined)
            queryData.data_source_id = updateData.data_source_id;
        if (updateData.query !== undefined)
            queryData.query = updateData.query;
        if (updateData.description !== undefined)
            queryData.description = updateData.description;
        if (updateData.options !== undefined)
            queryData.options = updateData.options;
        if (updateData.schedule !== undefined)
            queryData.schedule = updateData.schedule;
        if (updateData.tags !== undefined)
            queryData.tags = updateData.tags;
        if (updateData.is_archived !== undefined)
            queryData.is_archived = updateData.is_archived;
...
const executeRawQuerySchema = z.object({
    query: z.string(),
    dataSourceId: z.number(),
    parameters: z.record(z.any()).optional(),
    maxAge: z.number().optional().default(0)
});
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Execute a raw SQL query directly and return results",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: { type: "string", description: "SQL query to execute" },
                        dataSourceId: { type: "number", description: "ID of the data source to execute the query on" },
                        parameters: {
                            type: "object",
                            description: "Parameters to pass to the query (if any)",
                            additionalProperties: true
                        },
                        maxAge: { type: "number", description: "Maximum age of cached results in seconds (default: 0)" }
                    },
    ...
    try {
        ...
        switch (name) {
            ...
            case "execute_raw_query":
                logger.debug(`Handling execute_raw_query`);
                return await executeRawQuery(executeRawQuerySchema.parse(args));
